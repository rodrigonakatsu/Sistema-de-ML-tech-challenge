"""
Pipeline de Machine Learning para Previsão de Obesidade
Autor: Cientista de Dados - Hospital
Data: 2025-10-21

Este script contém toda a pipeline de ML:
...
print("="*80)
print("PIPELINE DE MACHINE LEARNING - PREVISÃO DE OBESIDADE")
print("="*80)

# 1. CARREGAMENTO DOS DADOS
print("\n[1/7] Carregando dados...")
df = pd.read_csv('Obesity.csv')
print(f"✓ Dataset carregado: {df.shape[0]} registros, {df.shape[1]} features")
...
# Score de hábitos saudáveis (quanto maior, mais saudável)
df_processed['Healthy_Score'] = (
    df_processed['FCVC'] +  # Consumo de vegetais
    df_processed['CH2O'] +  # Consumo de água
    df_processed['FAF'] -   # Atividade física
    (df_processed['TUE'] / 2)  # Menos tempo em tecnologia
)

# Score de risco (quanto maior, mais risco)
df_processed['Risk_Score'] = (
    df_processed['FAVC'].map({'yes': 1, 'no': 0}) +  # Comida calórica
    df_processed['family_history'].map({'yes': 1, 'no': 0}) +  # Histórico familiar
    (df_processed['NCP'] > 3).astype(int) +  # Muitas refeições
    df_processed['SMOKE'].map({'yes': 1, 'no': 0})  # Fumar
)

print(f"✓ Features criadas: BMI, Weight_Age_Ratio, Age_Category, Healthy_Score, Risk_Score")
...
# 3.3 Separação de features e target
...
print(f"✓ Target encoding: {len(le_target.classes_)} classes")
...
print(f"✓ Features normalizadas: {X_scaled.shape[1]} features")
...
print("\n[5/7] Treinamento e Avaliação de Modelos...")
...
print("✓ Modelo salvo: modelo_obesidade.pkl")

# Salvar encoders e scaler
joblib.dump(label_encoders, 'label_encoders.pkl')
print("✓ Label encoders salvos: label_encoders.pkl")

joblib.dump(le_target, 'target_encoder.pkl')
print("✓ Target encoder salvo: target_encoder.pkl")

joblib.dump(scaler, 'scaler.pkl')
print("✓ Scaler salvo: scaler.pkl")

# Salvar features para referência
feature_names = X.columns.tolist()
joblib.dump(feature_names, 'feature_names.pkl')
print("✓ Feature names salvas: feature_names.pkl")
...
print("✓ Dados processados salvos: dados_processados.csv")
...
if hasattr(best_model, 'feature_importances_'):
    ...
    print("✓ Feature importance salva: feature_importance.csv")

print("\n" + "="*80)
print("✅ PIPELINE CONCLUÍDA COM SUCESSO!")
print(f"✅ Modelo final: {best_model_name} com {best_accuracy*100:.2f}% de acurácia")
print("✅ Todos os artefatos foram salvos e estão prontos para deploy")
print("="*80)
...
for name, result in sorted(results.items(), key=lambda x: x[1]['accuracy'], reverse=True):
    print(f"{name:25s} | Acurácia: {result['accuracy']:.4f} ({result['accuracy']*100:.2f}%)")
print("-" * 80)
